...
#### An architecture for a digital public assistant
The assistant described above is largely possible to implement with existing technologies. We outline a reference architecture and implementation details for such an assistant, along with an accompanying prototype.

The three primary components are:
(1) A service guide, which interacts with residents to best connect them to relevant resources and guide them through the processes necessary to access those resources.
(2) An information manager, which gathers and organizes documents and information necessary to complete various processes and access services.
(3) A rules engine, which makes policy and process details reliably legible to other components of the system.
